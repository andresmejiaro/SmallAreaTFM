...
one_boot2=function(model_=model1,population_=SB11_20212,sample_=sample1,
  ...
  model_s_depto=create_model_dpto(sample = sample_s_,
                                  otras_var = otras_var,
                                  var_text_name = "y_syntethic")
  
  model_s_mcpio=create_model_mcpio(sample = sample_s_,
                                  otras_var = otras_var,
                                  var_text_name = "y_syntethic")
  ...
  ab_mcpio=create_estimates_level(mmodel = model_s_mcpio,
                            r_sample = sample_s_,
                            var_text_name = "y_syntethic",
                            orsample = sample_s_,
                            population_=population_s_)
  ab_mcpio %<>% ungroup %>% select(ESTU_COD_RESIDE_DEPTO,ESTU_COD_RESIDE_MCPIO,
                             MeanPred_mpio=MeanPred)
  ab %<>% left_join(ab_mcpio,by=c("ESTU_COD_RESIDE_DEPTO","ESTU_COD_RESIDE_MCPIO")) 
  
  
  ab_dpto=create_estimates_level(mmodel = model_s_depto,
                                  r_sample = sample_s_,
                                  var_text_name = "y_syntethic",
                                  orsample = sample_s_,
                                  population_=population_s_)
  ab_dpto %<>% ungroup %>% select(ESTU_COD_RESIDE_DEPTO,ESTU_COD_RESIDE_MCPIO,
                                   MeanPred_dpto=MeanPred)
  ab %<>% left_join(ab_dpto,by=c("ESTU_COD_RESIDE_DEPTO","ESTU_COD_RESIDE_MCPIO")) 
...
for(i in 1:100){
  tic=Sys.time()
  bootstrap_=future_map_dfr(1:100, ~one_boot2(
  ...
  fwrite(bootstrap_,paste0("model_based_experiment",i,".csv"))
