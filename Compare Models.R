...
countna=function(textcha){
  sum(is.na(SB11_20212[textcha]))
}


SB11_20212$FAMI_TIENEINTERNET %<>% replace_na("NA") 
SB11_20212$FAMI_TIENESERVICIOTV %<>% replace_na("NA") 
SB11_20212$FAMI_TIENECOMPUTADOR %<>% replace_na("NA") 
SB11_20212$FAMI_TIENELAVADORA %<>% replace_na("NA") 
SB11_20212$FAMI_TIENEHORNOMICROOGAS %<>% replace_na("NA") 
SB11_20212$FAMI_TIENEAUTOMOVIL %<>% replace_na("NA") 
SB11_20212$FAMI_TIENEMOTOCICLETA %<>% replace_na("NA") 
SB11_20212$FAMI_TIENECONSOLAVIDEOJUEGOS %<>% replace_na("NA") 
SB11_20212$FAMI_COMELECHEDERIVADOS %<>% replace_na("NA") 
SB11_20212$FAMI_COMECARNEPESCADOHUEVO %<>% replace_na("NA")
SB11_20212$FAMI_COMECEREALFRUTOSLEGUMBRE %<>% replace_na("NA")
SB11_20212$COLE_CARACTER %<>% replace_na("NA")


countna("FAMI_EDUCACIONMADRE")
...
create_model_mcpio=function(sample,var_text_name,otras_var){
  otras_var=paste(otras_var,collapse = "+")
  if(str_length(otras_var)==0){
    formula_e=paste0(var_text_name,"~"," (1|ESTU_COD_RESIDE_MCPIO")
  }else{
    formula_e=paste0(var_text_name,"~",otras_var,"+ (1|ESTU_COD_RESIDE_MCPIO)")
  }
...
create_model_dpto=function(sample,var_text_name,otras_var){
  otras_var=paste(otras_var,collapse = "+")
  if(str_length(otras_var)==0){
    formula_e=paste0(var_text_name,"~"," (1|ESTU_COD_RESIDE_DEPTO")
  }else{
    formula_e=paste0(var_text_name,"~",otras_var,"+ (1|ESTU_COD_RESIDE_DEPTO)")
  }
...
create_estimates_level=function(mmodel,r_sample,var_text_name,orsample,
  ...
  Datanew=population_
  Datanew %<>%  
    select(all_of(unique(c(mmodel@frame %>% names,
                           "ESTU_CONSECUTIVO","ESTU_COD_RESIDE_DEPTO","ESTU_COD_RESIDE_MCPIO")))) 
  ...
  s1=Datanew %>% group_by(ESTU_COD_RESIDE_MCPIO) %>% 
...
one_boot=function(sample1,otras_var=otras_var,var_text_name,rec=1,population_){
  ...
  ###
  model2=create_model_mcpio(sample = sample1,otras_var = otras_var,var_text_name = var_text_name)
  ab1=create_estimates_level(mmodel = model2,
                             r_sample = sample1,
                             var_text_name = var_text_name,
                             orsample = sample1,population_ = population_)
  
  ###
  model3=create_model_dpto(sample = sample1,otras_var = otras_var,var_text_name = var_text_name)
  ab2=create_estimates_level(mmodel = model3,
                             r_sample = sample1,
                             var_text_name = var_text_name,
                             orsample = sample1,population_ = population_)
  ...
  ab %<>% ungroup %>% 
    select(ESTU_COD_RESIDE_DEPTO,ESTU_COD_RESIDE_MCPIO,MB_Dept_Mpio=MeanPred)
  ab1 %<>% ungroup %>% 
    select(ESTU_COD_RESIDE_DEPTO,ESTU_COD_RESIDE_MCPIO,MB_Mpio=MeanPred)
  ab2 %<>% ungroup %>% 
    select(ESTU_COD_RESIDE_DEPTO,ESTU_COD_RESIDE_MCPIO,MB_Dept=MeanPred)
  
  
  ab=left_join(ab,ab1,by=c("ESTU_COD_RESIDE_MCPIO")) %>%
    left_join(ab2,by=c("ESTU_COD_RESIDE_MCPIO"))
  ab$rec=rec
  
  left_join(ab,HT,by=c("ESTU_COD_RESIDE_MCPIO"))
}
...
simulations=one_boot(sample1 = sample1,otras_var = otras_var,rec=1,
         var_text_name = var_text_name,population_ = population2)



Real=SB11_20212 %>% group_by(ESTU_COD_RESIDE_DEPTO,ESTU_COD_RESIDE_MCPIO) %>% 
  summarise(RealVal=mean(PUNT_GLOBAL))


simulations %<>% select(ESTU_COD_RESIDE_DEPTO=ESTU_COD_RESIDE_DEPTO.x,
                        ESTU_COD_RESIDE_MCPIO,MB_Dept_Mpio,MB_Mpio,MB_Dept,rec,HT_mean)

simulations %<>% left_join(Real) 

Depto_Mpio_names=SB11_20212 %>% select(ESTU_COD_RESIDE_DEPTO,ESTU_COD_RESIDE_MCPIO,ESTU_DEPTO_RESIDE,ESTU_MCPIO_RESIDE) %>% distinct()


simulations=left_join(Depto_Mpio_names,simulations)

### Plots ch 4


sample_sizes=sample1 %>% 
  group_by(ESTU_COD_RESIDE_DEPTO,ESTU_COD_RESIDE_MCPIO) %>%
  summarise(sample_size=n())


population_sizes=SB11_20212 %>% 
  group_by(ESTU_COD_RESIDE_DEPTO,ESTU_COD_RESIDE_MCPIO) %>%
  summarise(population_sizes=n())


simulation_plot=simulations %>% pivot_longer(cols = 
                                c("MB_Dept_Mpio","MB_Mpio","MB_Dept","HT_mean"),
                              names_to="estimador") 


simulation_plot=left_join(simulation_plot,sample_sizes)

simulation_plot$estimador %<>% str_replace_all("HT_mean","Direct Estimator")
simulation_plot$estimador %<>% str_replace_all("MB_Dept_Mpio","Model Based Department\nand Municipality")
simulation_plot$estimador %<>% str_replace_all("MB_Dept","Model Based only Department")
simulation_plot$estimador %<>% str_replace_all("MB_Mpio","Model Based only Municipality")

ggplot(data=simulation_plot,aes(x=abs(value-RealVal),color=estimador))+geom_density()+
  theme(legend.position = c(0.65,0.75))+labs(x="Absolute Error",y="Density",color="Estimator")

ggplot(data=simulation_plot,aes(y=abs(value-RealVal),color=estimador,x=log(sample_size)))+
  geom_point()+facet_wrap(~estimador)+theme(legend.position = "none")+
  labs(x="Logarithm of Area Sample Size",y="Absolute Error")


ggplot(data=simulation_plot,aes(x=abs(value-RealVal)/RealVal,color=estimador))+
  geom_density()+
  theme(legend.position = c(0.65,0.75))+
  labs(x="Relative Error",y="Density",color="Estimator")

ggplot(data=simulation_plot,
       aes(y=abs(value-RealVal)/RealVal,color=estimador,x=log(sample_size)))+
  geom_point()+facet_wrap(~estimador)+theme(legend.position = "none")+
  labs(x="Logarithm of Area Sample Size",y="Relative Error")

simulations=left_join(simulations,sample_sizes)

simulations=left_join(simulations,population_sizes)
...
model1@u %>%  qqnorm
residualsmodel=residuals(model1)

residualsmodel %<>% as_tibble()

library(gridExtra)


plot1=ggplot(data=residualsmodel,aes(x=value))+geom_histogram()+
  labs(x="Residual",y=NULL)
plot2=ggplot(data=residualsmodel,aes(sample=value))+geom_qq()+geom_qq_line()+
...
EfectoMunicipio=w$`ESTU_COD_RESIDE_MCPIO:ESTU_COD_RESIDE_DEPTO`



EfectoMunicipio %<>% rownames_to_column()

EfectoMunicipio %<>% select(valor=`(Intercept)`,MPIO=rowname)

EfectoMunicipio %<>% distinct() 

plot1=ggplot(data=EfectoMunicipio,aes(x=valor))+geom_histogram()+
  labs(x="Municipality Effect",y=NULL)
plot2=ggplot(data=EfectoMunicipio,aes(sample=valor))+geom_qq()+geom_qq_line()+
  labs(x="Theoretical Quantiles",y="Sample Quantiles")
...
EfectoDPTO=w$ESTU_COD_RESIDE_DEPTO



EfectoDPTO %<>% rownames_to_column()

EfectoDPTO %<>% select(valor=`(Intercept)`,DPTO=rowname)

EfectoDPTO %<>% distinct() 



plot1=ggplot(data=EfectoDPTO,aes(x=valor))+geom_histogram(bins=9)+
  labs(x="Department Effect",y=NULL)
plot2=ggplot(data=EfectoDPTO,aes(sample=valor))+geom_qq()+geom_qq_line()+
  labs(x="Theoretical Quantiles",y="Sample Quantiles")
