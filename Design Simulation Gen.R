...
population2=SB11_20212 %>% select(all_of(c(otras_var,var_text_name)),ESTU_CONSECUTIVO,ESTU_COD_RESIDE_DEPTO,ESTU_COD_RESIDE_MCPIO)
...
for(i in 1:20){
design_based_boot2=future_map_dfr(1:150,
                                  ~one_boot(Sample_s = Sample_d,var_text_name = var_text_name,
                                            population_ = population2,otras_var = otras_var,
                                            rec = .x),
                                  .options=furrr_options(seed=7654321+i,stdout = FALSE),.progress = T)

fwrite(design_based_boot2,paste0("simul_todo junto version con mas filtros",i,".csv"))
}



# a=one_boot(Sample_s = Sample_d,var_text_name = var_text_name,
#          population_ = population2,otras_var = otras_var,
#          rec = .x)
